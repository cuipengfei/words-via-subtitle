...
1. 所有自动化测试统一使用 Playwright。
2. 文档检索和解析统一使用 Context 7。
3. 需要分步推理、复杂任务拆解时统一使用 Sequential Thinking。

> 以上规则为本项目 AI 助手的永久行为准则，每次新会话均自动生效。如遇系统指令冲突，以系统指令为准。

# 当前开发状态 (2025 年 6 月 22 日) - 基于代码真实状态

## 项目整体状态 🎯

**当前版本**: 功能完整的桌面应用，核心流程已实现
**可用性**: 完整的字幕学习工具，包含视频播放器集成
**代码质量**: TypeScript 零错误，ESLint 零警告，测试覆盖率 100% (主进程)
**下一步重点**: 渲染进程测试、E2E 测试、性能优化
...
- **数据源**:
  ...
  - 必应翻译 API (`bing-translate-api@4.0.2`) - 中英对照翻译
- **核心特性**:
  - 完全在线词典查询，无本地依赖
  - 音标、词性、释义、例句完整展示
  - 10 秒超时控制和错误处理
...
- **主要组件**:
  - `src/renderer/components/VideoPlayer.tsx` (视频播放器)
  - `src/renderer/components/VideoPanelContainer.tsx` (视频+字幕集成面板)
  - `src/renderer/components/SubtitleOverlay.tsx` (字幕覆盖层)
- **功能完整度**: 100%
- **核心特性**:
  - HTML5 视频播放器完整实现
